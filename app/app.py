...
def index():
    ...
    cursor.execute('SELECT * FROM mlbplayersData')
...
def record_view(mlb_id):
    cursor = mysql.get_db().cursor()
    cursor.execute('SELECT * FROM mlbplayersData WHERE id=%s', mlb_id)
    result = cursor.fetchall()
...
def form_update_post(mlb_id):
    ...
    sql_update_query = """UPDATE mlbplayersData t SET t.Name = %s, t.Team = %s, t.Position = %s, t.Height_inches = 
...
def form_insert_post():
    ...
    sql_insert_query = """INSERT INTO mlbplayersData (Name, Team, Position, Height_inches, Weight_lbs, Age) VALUES (%s, %s,
    %s, %s, %s, %s) """
